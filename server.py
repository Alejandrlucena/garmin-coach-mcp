...
async def download_activity_fit(request: Request) -> Response:
    ...
    return JSONResponse({"error": "No hay tokens locales válidos para descargar esta actividad."}, status_code=503)
...
async def list_activities_web(request: Request) -> JSONResponse:
    ...
    if RAILWAY_FALLBACK_URL:
        try:
            ...
            return JSONResponse({"error": f"Sin tokens locales y el fallback no responde: {exc}"}, status_code=503)
    return JSONResponse({"error": "Sin tokens locales válidos."}, status_code=503)
